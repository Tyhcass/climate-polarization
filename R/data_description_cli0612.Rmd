...
surveys <-  read_csv(here::here("rawdata/climate_index_cli.csv"),
                       col_types = "cccc")[c(1:26,29:141,145:455,458:480),]

dcpo_input_raw_cc <- read_csv(here::here("rawdata/dcpo_input_raw_cc.csv")) %>%
  select(-1)
...
dcpo_input_raw <- read_csv(here::here("rawdata", "dcpo_input_raw_cc.csv"),
                                      col_types = "cdcddcd") %>%
...
dcpo_input <- DCPOtools::format_dcpo(dcpo_input_raw1,
                                     ...
                                     scale_cp = 3)
save(dcpo_input, file = here::here("data/pure_climate", "dcpo_input.rda"))
...
results_path <- here::here(file.path("data/pure_climate", 
                                     iter, 
...
if (!exists("results_path")) {
  results_path <- here::here("data/pure_climate", "2000", latest)
  ...
  if (!file.exists(file.path(results_path, paste0("dcpo-", latest, "-1.csv")))) {
    ...
    osf_retrieve_node("") %>% 
      ...
      osf_download(path = here::here("data/pure_climate", "2000"))
  }
  
  if(!file.exists(here::here("data/pure_climate", "dcpo_output_2000.rds"))) {
    dcpo_output <- as_cmdstan_fit(paste0(results_path, "/",
                                         list.files(results_path, pattern = "csv$")))
    
    dcpo_output$save_object(here::here("data/pure_climate", "dcpo_output_2000.rds"))
  }
}
...
theta_summary <- DCPOtools::summarize_dcpo_results(dcpo_input,
                                                   dcpo_output,
                                                   "theta")
mean(theta_summary$rhat) #1.000992
min(theta_summary$ess_bulk) #690.5938
mean(theta_summary$ess_bulk) #4201.05
min(theta_summary$ess_tail) #1173.562
mean(theta_summary$ess_tail) #2998.668
...
res_c <- theta_summary %>% 
  ...
  unique() %>% 
  length()
save(theta_summary, file = here::here("data/pure_climate","theta_summary_2000.rda"))
...
save(theta_results, file = here::here("data/pure_climate","theta_results_2000.rda"))
...
write.csv(items_summary, here::here("output","purecli_items_summary.csv"))

```

```{r list}
min(dcpo_input$data$year) #1992
max(dcpo_input$data$year) #2022
...
ls_year <- 1992:2022
ls_country <- dcpo_input$data$country %>% unique()
first_year <- min(dcpo_input$data$year)

cli_firstyr <- dcpo_input$data %>%
  dplyr::select(c("country", "year")) %>%
  unique()  %>% 
  dplyr::group_by(country) %>%
  transmute(country = country,
            cli_firstyear = min(year)) %>%
  unique() #1990

save(theta_results_list,
     file = here::here("output", "pure_climate/theta_results_list.rda"))  
...
internal_eco_dat <- dcpo_input$data %>% 
  ...
  mutate(prop = y_r/n_r,
         ...
         prop_90 = prop + qnorm(.9)*se,
         prop_10 = prop - qnorm(.9)*se) %>% 
  left_join(theta_summary %>% select(-kk, -tt), by = c("country", "year"))  #gen_clicon_4 3 or higher   #sup_govation_ 2  0.66 
...
internal_klg_plot #small relationship...
...
internal_policysupport <- dcpo_input$data %>% 
  filter(item == "sup_govaction_2 2 or higher" & year == 2017) %>% 
  mutate(iso2c = countrycode::countrycode(country,
                                          origin = "country.name",
                                          destination = "iso2c"),   #climate_chg_4 3 or higher climate_chg_4 4 or higher --0.4-0.5  #sup_govaction_5 4 or higher small ob  sup_govaction_2 2 or higher
         prop = y_r/n_r,
         se = sqrt((prop*(1-prop))/n),
         prop_90 = prop + qnorm(.9)*se,
         prop_10 = prop - qnorm(.9)*se) %>% 
...
t <- dcpo_input$data  %>% filter(country == "United States") %>% group_by(item, year) %>% summarize (n = n())


internal_us <- dcpo_input$data %>% 
  filter(item == "sup_govaction_4 3 or higher") %>% 
  mutate(prop = y_r/n_r,
         se = sqrt((prop*(1-prop))/n),
         prop_90 = prop + qnorm(.9)*se,
         prop_10 = prop - qnorm(.9)*se) %>% 
...
internal_us_label <- tibble(year = 1995, mean = .57, label = internal_us_cor)
...
internal_ge_label <- tibble(year = 1998, mean = .57, label = internal_ge_cor)
...
disaster <- read.csv(here::here("data","panel_0307.csv")) %>%
  ...
  mutate(country = countrycode::countrycode(country,
         ...
         iso2c = countrycode::countrycode(country,
                                          origin = "country.name",
                                          destination = "iso2c")) %>%
  mutate(iso2c = ifelse(country == "Kosovo", "XK", iso2c))  %>%
  group_by(country) %>%
  arrange(year, .by_group = T) %>%
  mutate(laws.year = laws - dplyr::lag(laws,  n = 1, order_by = year), 
         laws.econw.year = laws.econw - dplyr::lag(laws.econw,  n = 1, order_by = year) ) %>%
...
year_flood <- disaster %>%
  select(year, emdat.flood)  %>%
  group_by(year) %>%
  summarise(n = sum(emdat.flood)) %>%
  arrange(desc(n))  ## 2006,  2021, 2007, 2005, 2015, 2016

cc_flood <- theta_results %>%
  dplyr::inner_join(disaster %>%
                      select(country, year, emdat.flood),
                    by = c("country", "year"), multiple = "all")  %>% 
  select(theta, emdat.flood, draw, year) %>% 
  filter(!(is.na(theta)|is.na(emdat.flood))) %>% 
  nest(data = c(theta, emdat.flood))  


cc_flood_yr <- cc_flood %>%
  filter(year == 2015)  #2015 2016  2020 2011


cc_flood_cor <- map(cc_flood_yr$data, function(x) with(x, cor(theta, emdat.flood, use = "pairwise.complete.obs")))  %>% 
  unlist()  %>% 
  mean() %>% 
  round(2) %>% 
  paste0("R = ", .)  #0.22
...
flood_label <- tibble(mean = 0.4, emdat.flood = 10, label = cc_flood_cor)

flood_plot <- ggplot(disaster  %>% filter(year==2015),
                     aes(x = mean,
                         y = emdat.flood)) +
  geom_segment(aes(x = q10, xend = q90,
                   y = emdat.flood, yend = emdat.flood),
               na.rm = TRUE,
               alpha = .2) +
  geom_text(aes(label = iso2c), size = 1.5) +
  geom_smooth(method = 'lm', se = FALSE) +
  theme_bw() + theme(legend.position="none",
                     axis.text  = element_text(size=8),
                     axis.title = element_text(size=9)) +
  coord_cartesian(xlim=c(0.25, 0.8)) +
  labs(x = "Climate Change Attitude, 2015", y = "Number of floods, 2015") +
  geom_label(data = flood_label, aes(label = label))  

flood_plot

#########storm
year_strom <- disaster %>%
  select(year, emdat.storm)  %>%
  group_by(year) %>%
  summarise(n = sum(emdat.storm)) %>%
  arrange(desc(n))  ## 2005, 87 2020 58

cc_storm <- theta_results %>%
  dplyr::inner_join(disaster %>%
                      select(country, year, emdat.storm),
                    by = c("country", "year"), multiple = "all")  %>% 
  select(theta, emdat.storm, draw, year) %>% 
  filter(!(is.na(theta)|is.na(emdat.storm))) %>% 
  nest(data = c(theta, emdat.storm))  


cc_storm2010 <- cc_storm %>%
  filter(year == 2015)  #2010 0.28

cc_storm_cor <- map(cc_storm2010$data, function(x) with(x, cor(theta, emdat.storm, use = "pairwise.complete.obs")))  %>% 
  unlist()  %>% 
  mean() %>% 
  round(2) %>% 
  paste0("R = ", .)  #0.21

storm_label <- tibble(mean = 0.25, emdat.storm = 8, label = cc_storm_cor)

storm_plot <- ggplot(disaster  %>% filter(year==2015),
                     aes(x = mean,
                         y = emdat.storm)) +
  geom_segment(aes(x = q10, xend = q90,
                   y = emdat.storm, yend = emdat.storm),
               na.rm = TRUE,
               alpha = .2) +
  geom_text(aes(label = iso2c), size = 1.5) +
  geom_smooth(method = 'lm', se = FALSE) +
  theme_bw() + theme(legend.position="none",
                     ...
                     axis.title = element_text(size=9)) +
  coord_cartesian(xlim=c(0, 1)) +
  labs(x = "Climate Change Attitude, 2015", y = "Number of Storms, 2015") +
  geom_label(data = storm_label, aes(label = label))  
...
#####total
year_dis <- disaster %>%
  select(year, emdat.tot)  %>%
  group_by(year) %>%
  summarise(n = sum(emdat.tot)) %>%
  arrange(desc(n))  ## 2005 2010 2007 2015 2006 2002 2009 2014 2000

cc_dis <- theta_results %>%
  dplyr::inner_join(disaster %>%
                      select(country, year, emdat.tot),
                    by = c("country", "year"), multiple = "all")  %>% 
  select(theta, emdat.tot, draw, year) %>% 
  filter(!(is.na(theta)|is.na(emdat.tot))) %>% 
  nest(data = c(theta, emdat.tot))  


cc_tot <- cc_dis %>%
  filter(year == 2014)  #2014 0.22


cc_dis_cor <- map(cc_tot$data, function(x) with(x, cor(theta, emdat.tot, use = "pairwise.complete.obs")))  %>% 
  unlist()  %>% 
  mean() %>% 
  round(2) %>% 
  paste0("R = ", .)  #0.22
...
dis_label <- tibble(mean = 0.35, emdat.tot = 15, label = cc_dis_cor)

dis_plot <- ggplot(disaster  %>% filter(year==2014),
                   aes(x = mean,
                       y = emdat.tot)) +
  geom_segment(aes(x = q10, xend = q90,
                   y = emdat.tot, yend = emdat.tot),
               na.rm = TRUE,
               alpha = .2) +
  geom_text(aes(label = iso2c), size = 1.5) +
  geom_smooth(method = 'lm', se = FALSE) +
  ...
  labs(x = "Climate Change Attitude, 2014", y = "Number of Storms and floods, 2014") +
  geom_label(data = dis_label, aes(label = label))  
...
year_law <- disaster %>%
  select(year, laws.econw)  %>%
  group_by(year) %>%
  summarise(n = sum(laws.econw)) %>%
  arrange(desc(n))  ## 2017--2021

cc_ecolaw <- theta_results %>%
  dplyr::inner_join(disaster %>%
                      select(country, year, laws.econw),
                    by = c("country", "year"), multiple = "all")  %>% 
  select(theta, laws.econw, draw, year) %>% 
  filter(!(is.na(theta)|is.na(laws.econw))) %>% 
  nest(data = c(theta, laws.econw))  


cc_ecolaw_yr <- cc_ecolaw %>%
  filter(year == 2021)  #2021 0.37


cc_ecolaw_cor <- map(cc_ecolaw_yr$data, function(x) with(x, cor(theta, laws.econw, use = "pairwise.complete.obs")))  %>% 
  unlist()  %>% 
  mean() %>% 
  round(2) %>% 
  paste0("R = ", .)  #

ecolaw_label <- tibble(mean = 0.25, laws.econw = 10, label = cc_ecolaw_cor)

ecolaw_plot <- ggplot(disaster  %>% filter(year==2021) ,
                      aes(x = mean,
                          y = laws.econw)) +
  geom_segment(aes(x = q10, xend = q90,
                   y = laws.econw, yend = laws.econw),
               na.rm = TRUE,
               alpha = .2) +
  geom_text(aes(label = iso2c), size = 2) +
  geom_smooth(method = 'lm', se = FALSE) +
  ...
  labs(x = "Climate Change Attitude, 2021", y = "Number of EcoLaws, 2021") +
  geom_label(data = ecolaw_label, aes(label = label))  

ecolaw_plot

####law

cc_law <- theta_results %>%
  dplyr::inner_join(disaster %>%
                      select(country, year, laws),
                    by = c("country", "year"), multiple = "all")  %>% 
  select(theta, laws, draw, year) %>% 
  filter(!(is.na(theta)|is.na(laws))) %>% 
  nest(data = c(theta, laws))  


cc_law_yr <- cc_law %>%
  filter(year == 2020)  #2021 0.34


cc_law_cor <- map(cc_law_yr$data, function(x) with(x, cor(theta, laws, use = "pairwise.complete.obs")))  %>% 
  unlist()  %>% 
  mean() %>% 
  round(2) %>% 
  paste0("R = ", .)  #0.34

law_label <- tibble(mean = 0.25, laws = 40, label = cc_law_cor)

law_plot <- ggplot(disaster  %>% filter(year==2020) ,
                   aes(x = mean,
                       y = laws)) +
  geom_segment(aes(x = q10, xend = q90,
                   y = laws, yend = laws),
  ...
  theme_bw() + theme(legend.position="none",
                     ...
                     axis.title = element_text(size=9)) +
  coord_cartesian(xlim=c(0.1, 0.75)) +
  labs(x = "Climate Change Attitude, 2020", y = "Number of Laws, 2020") +
  geom_label(data = law_label, aes(label = label))  
...
qog_ei <-  read_csv(here::here("data","qog_ei_ts_sept21.csv")) %>%
  select(cname, year,edgar_co2gdp, ccl_exepp, ccl_lpp, ccl_mitlpp, ccl_nexep, ccl_nmitlp) %>%
  transmute(country = countrycode::countrycode(cname,  origin = 'country.name', 'country.name'), 
            year = year, 
            co2gdp = edgar_co2gdp, 
            alllaw_yr = ccl_exepp, 
            law_total_yr = ccl_lpp, 
            miti_law_yr = ccl_mitlpp, 
            alllaw_cul = ccl_nexep, 
            miti_law_cul = ccl_nmitlp) %>%
  select(country, year, miti_law_yr) %>%
  mutate(
         iso2c = countrycode::countrycode(country,
                                          origin = "country.name",
                                          destination = "iso2c")) %>%
...
cc_miti <- theta_results %>%
  dplyr::inner_join(qog_ei %>%
                      select(country, year, miti_law_yr),
                    by = c("country", "year"), multiple = "all")  %>% 
  select(theta, miti_law_yr, draw, year) %>% 
  filter(!(is.na(theta)|is.na(miti_law_yr))) %>% 
  nest(data = c(theta, miti_law_yr))  


cc_mitilaw_yr <- cc_miti %>%
  filter(year == 2019)  #2021 0.38


cc_mitilaw_cor <- map(cc_mitilaw_yr$data, function(x) with(x, cor(theta, miti_law_yr, use = "pairwise.complete.obs")))  %>% 
  unlist()  %>% 
  mean() %>% 
  round(2) %>% 
  paste0("R = ", .)  #0.43

mitilaw_label <- tibble(mean = 0.25, miti_law_yr = 4, label = cc_mitilaw_cor)

mitilaw_plot <- ggplot(qog_ei  %>% filter(year==2019) ,
                   aes(x = mean,
                       y = miti_law_yr)) +
  geom_segment(aes(x = q10, xend = q90,
                   y = miti_law_yr, yend = miti_law_yr),
               na.rm = TRUE,
               alpha = .2) +
  geom_text(aes(label = iso2c), size = 2) +
  geom_smooth(method = 'lm', se = FALSE) +
  ...
  coord_cartesian(xlim=c(0.1, 0.75),
                  ylim = c(0,5)) +
  labs(x = "Climate Change Attitude, 2019", y = "Number of Mitigation Laws, 2019") +
  geom_label(data = mitilaw_label, aes(label = label))  

mitilaw_plot

(ecolaw_plot + law_plot)/  mitilaw_plot + patchwork::plot_annotation(caption = "Note: Gray whiskers and shading represent 90% credible intervals.")
#a little bit better than the whole data's measure #0.35

(storm_plot + flood_plot)/dis_plot  + patchwork::plot_annotation(caption = "Note: Gray whiskers and shading represent 90% credible intervals.")
# a little bit better than whole data, but still around 0.22
