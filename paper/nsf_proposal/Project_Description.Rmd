...
This project will pursue two additional avenues of data collection: single-country surveys and surveys that are not publicly available.
...
First, while some of these surveys, such as the U.S. General Social Survey, have been combined into time-series datasets including many years of surveys with standardized variable names, many of these surveys are scattered across datasets covering only a single year and, more often than not, inconsistent variable names.
...
Other, similar models of public opinion require either very dense survey data [@Mcgann2019; @Kolczynska2024] or ancillary data on population characteristics [@Berwick2024]. The available data on climate-change opinion, as on most topics, sparse rather than dense, and reliable ancillary data is not available for the broad sample of countries considered in this project.]
...
`DCPOtools` automates the process of collecting the necessary survey items from datasets and performs data wrangling computationally [@SoltEtAl2019].
